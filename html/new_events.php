...
<?php 
    ...
    $stmt = $pdo->prepare("SELECT * FROM new_events WHERE id = :id");
    $stmt->execute([':id' => $id]);
...
?>
  ...
  <div class="hero" style="background-image:linear-gradient(rgba(0,0,0,0), rgba(0,0,0,0.7)), url('/show_image.php?type=poster&tab=new&id=<?php echo $id;?>');">
...
<?php echo $row['what_to_expect'];?>
 ...
 <section class="event-section">
    ...
    <aside class="sidebar">
     ...
     <div class="sidebar-card" id="actionButtons" style="display: flex; gap: 10px;">
        <button class="btn success" style="flex: 1;" onclick="window.location.href='manage_event.php?cmd=accept&type=neve&id=<?php echo $row['id'];?>'">Accept</button>
        <button class="btn" style="flex: 1; background-color: #dc3545; color: #fff;" onclick="window.location.href='manage_event.php?cmd=del&type=neve&id=<?php echo $row['id'];?>'">Reject</button>
      </div>
