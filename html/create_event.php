...
<body>
  ...
  <header>
    <div class="nav">
      ...
      <div class="cta">
 <?php if($role==="organizer") echo '<a class="btn primary" href="logout.php">Logout</a><a class="btn secondary" href="organizer_dashboard.php">Organizer Panel</a>';
...
<form action="submit_event.php" method="POST" id="create-event-form" enctype="multipart/form-data">
          ...
          <div class="form-grid">
           ...
           <div class="form-group">
              ...
              <div class="radio-group">
                <label>
                  <input id='reg_form' type="radio" name="registration-method" value="form" checked>
                  <span>Standard Form</span>
                </label>
                <label>
                  <input type="radio" name="registration-method" value="qrcode">
                  <span>QR Code</span>
                </label>
              </div>
              
              <div class="file-upload-wrapper" id="qrcode-dropzone" style="display: none; margin-top: 16px;">
                <p>Click or drag & drop to upload</p>
  ...
  <script>
document.getElementById('reg_form').checked= true;
    const qrUploadZone = document.getElementById('qrcode-dropzone');
    const regMethodRadios = document.querySelectorAll('input[name="registration-method"]');

    regMethodRadios.forEach(radio => {
        radio.addEventListener('change', () => {
            if (radio.value === 'qrcode') {
                qrUploadZone.style.display = 'block'; // Show the QR upload box
            } else {
                qrUploadZone.style.display = 'none'; // Hide the QR upload box
            }
        });
    });
function save_event(){
